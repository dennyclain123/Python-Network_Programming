...
try:
    ...
    while True:
            ...
            #Spliting Octets like 192.168.1.1            
            ...
            #This condition checking is from stackoverflow    
                break
            
            else:
                print("\nThe IP address is INVALID! Please retry!\n")
                continue
    #Part 2, Request Subnet Mask from user
    #mask = input("Enter Subnet Mask: ")
    #prefix = sum(bin(int(x)).count('1') for x in mask.split('.')) #copied from stackoverflow
    #print (prefix)
    ...
    while True:
        ...
        #Spliting Octets like 255.255.255.0            
        ...
        if (len(mask_octets) == 4) and (int(mask_octets[0]) == 255) and (int(mask_octets[1]) in masks) and (int(mask_octets[2]) in masks) and (int(mask_octets[3]) in masks) and (int(mask_octets[0]) >= int(mask_octets[1]) >= int(mask_octets[2]) >= int(mask_octets[3])):
            #Also learn from stackoverflow :D
            break
            
        else:
            print("\nThe subnet mask is INVALID! Please retry!\n")
            continue
    #Algorithm for subnet identification, based on IP and Subnet Mask identities
    ...
    #For Example 255.255.255.0 => 11111111111111111111111100000000
        
    #Counting host bits in the mask and calculating number of hosts/subnet
    no_of_zeros = binary_mask.count("0")
    no_of_ones = 32 - no_of_zeros
    no_of_hosts = abs(2 ** no_of_zeros - 2) #subtract 2 from all subnets, for example /24 = 254, so 254 -2 252 is vaild hosts
    ...
    while True:
        ...
        if generate == "y":
            ...
            print("\nRandom IP address is: %s" % y_iaddr)
            print("\n")
            continue
                
        else:
            print((colored(figlet_format("Bye!!"), color="cyan")))
            break

except KeyboardInterrupt:
    print("\n\nProgram aborted by user. Aborting...\n")
    sys.exit()